...
class boardInfo:
    ...
    def isFree(self, x, y):
        return x >= 0 and y >= 0 and x < self.size[0] and y < self.size[1] and self.board[x][y] == 0
    ...
    def update_pointScore(self, x, y, role, radius=6):
        scale = self.size[0]
    ...
    #proof and disproof number version
    def checkmate(self,role,rule = 1, depth = 0, threshold = THRESHOLD):
    ...
    # output : p,dp:represent proof number and disproof number respectively
        ...
        son_list = []  
        move_list = []  
        activeMoveList = copy.deepcopy(self.activeMoves)
...
def choose_move(cur_board):
    esti = boardInfo(cur_board)    
    win_move_list, must = esti.checkmate(role = 1, threshold = 15)
    if win_move_list:
        if len(win_move_list) == 1 and must == 1 :
            f = open("C:/Users/Duspunk张/Desktop/人工智能/Final Project/GOMOKU/code/Alpha-Ji/abpruning1/ourlog.txt", 'w')
            f.close()
            return win_move_list[0]
        else:
            best_move = win_move_list[0]
